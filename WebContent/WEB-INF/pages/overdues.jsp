	...
	$(function(){
		$("#pageNo").change(function(){
			...
			if(reg.test(val)){
				...
				if(val >= 1 && val <= parseInt("${orderPage.totalPageNumber}")){
					flag = true;
				}
			}
			...
			//3.页面跳转
			var href = "order-getOverdues?pageNo=" + pageNo + "&" + $(":hidden").serialize();
					...
					<div class="row">
						<div class="col-md-6">
							...
							<div class="panel panel-headline">
								<form action="order-getOverdues" method="post">
										<label>单号：</label>
								...
								<div class="panel-body">
									<table class="table table-hover">
										...
										<tbody>
											<s:if test="#request.orderPage.list != null && #request.orderPage.list.size() != 0">
											...
											<tr>
												...
												<td>
													<a href="roomType-getRoomType?id=${order.room.roomType.id }">${order.room.roomName}</a>
												</td>
												...
												<td>
													<span class="label label-success">
														<a href="order-getRenewPage?id=${order.user.id }" style="color: white" class="renew">续订</a>
													</span>
													&nbsp;&nbsp;
													<span class="label label-danger">
														<a href="order-getCheckOutPage?id=${order.user.id }" class="checkOut" style="color: white">退房</a>
													</span>
												</td>
											</tr>
											</c:forEach>
									...
									<div class="row">
										<ul id="PageNum">
											共<font color="green" style="font-size: 15px;font:bold;"> ${orderPage.totalPageNumber }</font> 页&nbsp;&nbsp;当前第<font color="green" style="font-size: 15px;font:bold;"> ${orderPage.pageNo }</font> 页
											<li>
													<a href="order-getOverdues?pageNo=1">首页</a>
											</li>
											<li>
												<s:if test="#orderPage.hasPrev">
													<a href="order-getOverdues?pageNo=${orderPage.prevPage }">上一页</a>
												</s:if>
											</li>
											<c:forEach var="pageNum" begin="1" end = "${orderPage.totalPageNumber }"
														step = "1">
												<c:if test="${pageNum <= 6 }">
													<li>
														<a href="order-getOverdues?pageNo=${pageNum }">${pageNum }</a>
													</li>
												</c:if>
											</c:forEach>
											<li>
												<s:if test="#orderPage.hasNext">
													<a href="order-getOverdues?pageNo=${orderPage.nextPage }">下一页</a>
												</s:if>
											</li>
											<li>
												<a href="order-getOverdues?pageNo=${orderPage.totalPageNumber }">末页</a>
											</li>
