	...
	<script type="text/javascript">
		$(function(){
			$("#pageNo").change(function(){
				...
				//3.页面跳转
				var href = "room-getRooms?pageNo=" + pageNo + "&" + $(":hidden").serialize();
						...
						<div class="col-md-6">
							...
							<div class="panel">
								<div class="panel-heading">
									<h3 class="panel-title">客房信息</h3>
									<div class="right">
									</div>
								</div>
								<form action="room-getRooms" method="post">
										<label>房间编号：</label>
										<input type="text" placeholder="输入房间编号" name="model.id"/>
										<label>房间名：</label>
										<input type="text" placeholder="输入房间名" name="model.roomName"/>
										<label>状态：</label>
										<select placeholder=请选择房间名"" name="model.states">
											<option value=""></option>
									...
									<table class="table table-striped">
										...
										<tbody>
											...
											<tr>
												...
												<td>${id }</td>
												<td>
													<a href="order-getRoomOrder?type='int'&pageNo=${roomPage.pageNo }&model.room.id=${id}">
													${roomName }</a>
												</td>
												...
												<td>
													...
													<s:else>
														<span class="label label-danger">已售</span>
													</s:else>
												</td>
												<td>
													<s:if test="!states">
								...
								<div class="panel-footer">
									<div class="row">
										<ul id="PageNum">
											...
											<li>
													<a href="room-getRooms?pageNo=1">首页</a>
											</li>
											<li>
												<s:if test="#roomPage.hasPrev">
													<a href="room-getRooms?pageNo=${roomPage.prevPage }">上一页</a>
												</s:if>
											</li>
											<c:forEach var="pageNum" begin="1" end = "${roomPage.totalPageNumber }"
												...
												<c:if test="${pageNum <= 6 }">
													<li>
														<a href="room-getRooms?pageNo=${pageNum }">${pageNum }</a>
													</li>
												</c:if>
											</c:forEach>
											<li>
												<s:if test="#roomPage.hasNext">
													<a href="room-getRooms?pageNo=${roomPage.nextPage }">下一页</a>
												</s:if>
											</li>
											<li>
												<a href="room-getRooms?pageNo=${roomPage.totalPageNumber }">末页</a>
											</li>
