...
<script type="text/javascript">
	$(function(){
		$("#getUser").click(function(){
			...
			var url = "order-getRegisterRecord?id=" + id;
			...
			$.post(url,args,function(data){
				if(data != "0"){
					...
					if(backData.states != "0"){
						alert("用户 " + backData.name + " 账号无预定信息！");
						return false;
					}
					...
					//右边
					$("#roomId").val(backData.roomId);
					$("#roomName").html(backData.roomName);
					$("#peoples").html(backData.peoples);
					$("#price").val(backData.price);
					$("#net_info").html(backData.net_info);
					$("#bef_info").html(backData.bef_info);
					$("#departureTime").val(backData.departureTime);
					$("#registerTime").val(backData.registerTime);
					
					$("#other_info").val(backData.other_info);
					
					if(backData.park_info == "1")
						$("#park_info").html('<span class="label label-success">有车位</span>');
					else
						$("#park_info").html('<span class="label label-danger">无车位</span>');
					...
					$("#orderId").val(backData.orderId);
				}else{
					alert("不存在编号为"+ id +"的用户或无该用户预定信息");
					$("#id").val('');
				}
			});
			return false;
				...
				<div class="container-fluid bookNow">
					...
					<div class="row">
						...
						<div class="col-md-6" style="width: 50%">
							...
							<div class="panel">
								<div class="panel-heading">
									<h3 class="panel-title" id="roomNameTitle">客房信息</h3>
								</div>
								<div class="panel-body no-padding">
									<form action="order-checkIn" method="post">
									<table class="table">
										<tbody>
											<tr>
												<td><span class="label label-primary">房间编号</span></td>
												<td>
													<input type="text" id="roomId" name="room.id" value="${room.id }" readonly="readonly"/>
											...
											<tr>
												...
												<td  id="peoples">${room.roomType.peoples }</td>
												<td><span class="label label-primary">定价</span></td>
												<td>￥
													<input type="text" id="price" name="price" 
														readonly="readonly" size ="5" value="${room.roomType.price }"/>
												</td>
											</tr>
											<tr>
												<td><span class="label label-primary">上网</span></td>
												<td id="net_info">${room.roomType.net_info }</td>
												<td><span class="label label-primary">早餐</span></td>
												<td id="bef_info">${room.roomType.bef_info }</td>
											</tr>
											<tr style="height: 10%;overflow: hidden;">
												<td>
													<span class="label label-primary">预离时间</span>
													<br/><br/>
													 <span class="label label-primary">预定时间</span>
												</td>
												<td>
													...
													<br/><br/>
													<input type="text" placeholder="点击设置时间" id="registerTime" name="registerTime"
														readonly="readonly" style="width:160px;cursor:pointer;"/>
												</td>
											...
											<tr>
												...
												<td>
													<input type="text" name="id" id="forShowUserID" readonly="readonly"/>
												</td>
												<td></td>
												<td></td>
											</tr>
